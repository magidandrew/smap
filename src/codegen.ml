...
(*  | _ -> void_t (*add in prob, string, and list types later! *) *)
  ...
  SFunCall ("printint", [e])
    -> L.build_call printint_func [| (expr builder e) |]
       "printint" builder
  *)
  let genGlobalListLit (_typlist,exp) n m =
    let defaultZero =
    ...
    let wholeThing = (match exp with
      (SList_lit(inits))
      -> (match inits with
          ([A.Int],SInt_lit(_v))::_rest -> let one = L.const_int i32_t 1 in
  ...
  let _get_vals_func : L.llvalue = L.declare_function "get_vals" get_vals_t the_module in
  ...
  let _print_prob_int_debug_func : L.llvalue = L.declare_function "print_prob_int_debug" print_prob_int_debug_t the_module in

  let bad_add_head_t : L.lltype = L.var_arg_function_type i32_t [| L.pointer_type list_t; L.pointer_type i8_t |] in
  let _bad_add_head_func : L.llvalue = L.declare_function "bad_add_head" bad_add_head_t the_module in
  ...
  let local_vars =
    ...
    and add_local m (SVdecl((t, n), _exp)) = (* add locals to scope  *)
  ...
  *)
  ...
  let tmp = match (vtype) with
             ...
             | _ -> raise(Failure "tmp match failed")
  in
  (* malloc(sizeOf(int)) *)
  let heapAddr = match (vtype) with
                     ...
                     | [A.Bool] -> raise(Failure("fill in for bool"))
                     | [A.Prob] -> raise(Failure("fill in for prob someday... >.<\""))
                     | _ -> raise(Failure "heapAddr match failed")
  in
  ...
  let rec deRef lst handle =
    match lst with
    [] -> raise(Failure "deRef not matched")
    | ((typs,i)::rest) ->
    let elt = L.build_call get_at_func [| handle; expr builder (typs,i)|]
        ...
        | _ -> raise(Failure "elt not matched")
    in
    match e with
    SInt_lit i
    -> L.const_int i32_t i
    ...
    | SCast(_,_) -> raise(Failure "SCast not implemented")
    | SLength(p)
       ...
       (match eltType with
         ...
         -> let theList = expr builder listId in
            ...
            let eltAsPtr = match eltType with
                       ...
                       | [A.Bool] -> raise(Failure("fill in for bool"))
                       | A.Prob::_tl -> raise(Failure("fill in for prob someday... >.<\""))
                       | _ -> raise(Failure "eltAsPtr not matched")
                      in
    ...
    -> (match theTyp with
        ...
        let _aList = L.build_alloca (L.pointer_type list_t) "theList" in
        ...
        in anAllocatedList
        | [A.List;_u] (*simple list of one type - (no lists of lists here)*)
        -> (match elts with
            [] (*CASE: EMPTY LIST *)
            ->
            (* get pointer to list struct *)
    ...
    | _ -> raise(Failure "e2' not matched")
	  ) e1' e2' "tmp" builder
      | SBinop (e1, op, e2) ->
	  let e1' = expr builder e1
	  and e2' = expr builder e2 in
    ...
    -> (match e1 with
        ...
        -> let e1' = lookup nm
          ...
          (match op with (*PlusEqual | MinusEqual | TimesEqual | DivEqual | Equal*)
              A.PlusEqual -> raise (Failure("special assignments need binops to be able to work"))
            | A.MinusEqual -> raise (Failure("special assignments need binops to be able to work"))
            | A.TimesEqual -> raise (Failure("special assignments need binops to be able to work"))
            | A.DivEqual -> raise (Failure("special assignments need binops to be able to work"))
            | A.Equal -> ignore(L.build_store e2' e1' builder); e1')
        | (_ty, SListElement (listId,index,rest))
        -> (match rest with
             []
             ...
             -> let theList = expr builder listId in
                ...
                in listToIndexInto )
      |_-> raise(Failure "unmatched expr"))
    | SFunCall ("printint", [e])
    -> L.build_call printint_func [| (expr builder e) |]
       "printint" builder
  ...
  let rec stmt builder = function
    ...
    | SIf_Elif(if_stmt, fst_elif, _elif_lst) ->
  ...
  (* if conditional *)
      ...
      L.builder_at_end context merge_bb
  | _ -> raise(Failure "control not matched")
