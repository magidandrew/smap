; ModuleID = 'Smap'
source_filename = "Smap"

%list = type { i8**, i32, i32 }

@test = private unnamed_addr constant [17 x i8] c"test test test!\0A\00", align 1
@fmt = private unnamed_addr constant [4 x i8] c"%d\0A\00", align 1
@fmt.1 = private unnamed_addr constant [4 x i8] c"%f\0A\00", align 1
@test.2 = private unnamed_addr constant [17 x i8] c"test test test!\0A\00", align 1
@fmt.3 = private unnamed_addr constant [4 x i8] c"%d\0A\00", align 1
@fmt.4 = private unnamed_addr constant [4 x i8] c"%f\0A\00", align 1

declare i32 @printstr(i8*)

declare i32 @absolute(i32)

declare i32 @isCompDivisible(i32)

declare double @ceilFloat(double)

declare i32 @ascii(i8*)

declare i8* @int_to_char(i32)

declare i32 @stringLength(i8*)

declare i32 @stringReverse(i8*)

declare i32 @printb(i1)

declare { i8*, i32 } @testMakeStruct(i8*, i32)

declare i32 @printint(i32)

declare i32 @printf(i8*, ...)

declare i32 @bad_add_head(%list*, i8*, ...)

declare i32 @very_bad_get_head(%list*, ...)

define i32 @main() {
entry:
  %x = alloca i32
  store i32 10, i32* %x
  %x1 = load i32, i32* %x
  call void @rec_print(i32 %x1)
  ret i32 0
}

define void @rec_print(i32 %number) {
entry:
  %number1 = alloca i32
  store i32 %number, i32* %number1
  %next_number = alloca i32
  %number2 = load i32, i32* %number1
  %tmp = sub i32 %number2, 1
  store i32 %tmp, i32* %next_number
  br label %if

if:                                               ; preds = %entry
  %number5 = load i32, i32* %number1
  %tmp6 = icmp sgt i32 %number5, 0
  br i1 %tmp6, label %if_body, label %merge

if_body:                                          ; preds = %if
  %number3 = load i32, i32* %number1
  %printint = call i32 @printint(i32 %number3)
  %next_number4 = load i32, i32* %next_number
  call void @rec_print(i32 %next_number4)
  br label %merge

merge:                                            ; preds = %if, %if_body
  ret void
}
